...
#define LEAP (!(yr%4))/*Checking for leap year*/
...
void short_cal(int year, int month){
	...
	for (yr = 1900; yr <= year; yr++){/*Traversing years*/
		if (yr < year)
			m = 12; /*It will traverse all months if the year is not what user entered*/
		else
...
/*yr is for traversing the year, and year is the year given by the user.
mon is for traversing the months and month is the month given by user.
curr contains the starting day of the current month and next contains 
the starting day of the next month.*/
